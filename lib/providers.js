...
export async function fetchPlans(providerKey, token) {
    ...
    const normalizePlanLocations = (plan) => {
        const raw = plan.locations || plan.regions || plan.region_list || plan.available_regions || [];
        if (!Array.isArray(raw)) return [];

        return raw
            .map((item) => {
                if (typeof item === 'string') return item;
                if (item && typeof item === 'object') return item.id || item.region || item.code || item.slug || null;
                return null;
            })
            .filter(Boolean);
    };
    ...
    if (providerKey === 'vultr') {
        ...
        return (data.plans || data.plans || []).map(plan => ({
            ...
            locations: normalizePlanLocations(plan)
        }));
    }
    if (providerKey === 'digitalocean') {
