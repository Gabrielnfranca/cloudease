...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['upload'])) {
    ...
    header('Location: filemanager.php?path=' . urlencode($relCurrent) . '&view=' . urlencode($view));
    exit;
}
...
?>
<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Gerenciador de Arquivos</title>
<style>
    :root{--bg:#f3f4f8;--card:#fff;--line:#e5e7eb;--text:#111827;--muted:#6b7280;--brand:#6c4fd3;--brand2:#5a3ec7}
    *{box-sizing:border-box}
    ...
    .shell{display:grid;grid-template-columns:220px 1fr;min-height:calc(100vh - 42px)}
    .sidebar{background:#fff;border-right:1px solid var(--line);padding:10px}
    .tree-title{font-size:12px;color:var(--muted);margin:4px 0 8px}
    .tree{list-style:none;padding:0;margin:0}
    .tree li{padding:5px 0;font-size:13px}
    .tree a{text-decoration:none;color:#374151}
    .main{padding:12px}
    .toolbar{display:flex;gap:14px;flex-wrap:wrap;align-items:center;justify-content:space-between;background:#fff;border:1px solid var(--line);padding:12px;border-radius:10px}
    ...
    .grid{margin-top:10px;display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:10px}
    ...
    @media (max-width:900px){.shell{grid-template-columns:1fr}.sidebar{border-right:none;border-bottom:1px solid var(--line)}.toolbar{flex-direction:column;align-items:stretch}.toolbar-left,.toolbar-right{width:100%}}
</style></head><body>
<div class="topbar">
    <div><strong>CloudEase File Manager</strong> <span class="site-badge">Site: <?=esc($siteName)?></span> / <?=esc($rel ?: '/')?></div>
    <div class="actions">
...
<div class="shell">
    <aside class="sidebar">
        ...
        <ul class="tree">
            <li><a href="filemanager.php?view=<?=urlencode($view)?>">📁 /</a></li>
            <?php foreach($treeItems as $dir): if($dir==='.'||$dir==='..') continue; $fullBase=$base.'/'.$dir; if(!is_dir($fullBase)) continue; ?>
            <li><a href="?path=<?=urlencode($dir)?>&view=<?=urlencode($view)?>">📁 <?=esc($dir)?></a></li>
            <?php endforeach; ?>
        </ul>
    </aside>
    <main class="main">
        <form class="toolbar" method="post" enctype="multipart/form-data">
            ...
            <div class="toolbar-right">
                <?php if($rel!==''){ $up = dirname($rel); if($up==='.')$up=''; echo '<a class="btn gray" href="?path='.urlencode($up).'&view='.urlencode($view).'">Voltar</a>'; } ?>
                <a class="btn gray" href="?path=<?=urlencode($rel)?>&view=<?=urlencode($view)?>">Atualizar</a>
                <div class="view-switch">
                    <a class="icon-btn <?= $view === 'grid' ? 'active' : '' ?>" href="?path=<?=urlencode($rel)?>&view=grid" title="Modo grade" aria-label="Modo grade">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>
                    </a>
                    <a class="icon-btn <?= $view === 'list' ? 'active' : '' ?>" href="?path=<?=urlencode($rel)?>&view=list" title="Modo detalhes" aria-label="Modo detalhes">
        ...
        <div class="list">
            <div class="list-header"><div>Nome</div><div>Tipo</div><div>Tamanho</div><div>Acoes</div></div>
            <?php foreach($visibleItems as $it): $full=$current.'/'.$it; $isDir=is_dir($full); $next=trim(($rel? $rel.'/' : '').$it,'/'); $size=$isDir ? '-' : (filesize($full).' bytes'); ?>
            <div class="list-row file-item" data-name="<?=esc($it)?>" data-isdir="<?= $isDir ? '1' : '0' ?>" data-open-url="<?= $isDir ? ('?path='.urlencode($next).'&view=list') : esc(($rel? $rel.'/' : '').$it) ?>" data-delete-url="?path=<?=urlencode($rel)?>&view=list&delete=<?=urlencode($it)?>">
                <div><?php if($isDir): ?>📁 <a href="?path=<?=urlencode($next)?>&view=list"><?=esc($it)?></a><?php else: ?>📄 <a href="<?=esc(($rel? $rel.'/' : '').$it)?>" target="_blank"><?=esc($it)?></a><?php endif; ?></div>
                ...
                <div>
                    ...
                    <a class="danger" href="?path=<?=urlencode($rel)?>&view=list&delete=<?=urlencode($it)?>" onclick="return confirm('Excluir este item?')">Excluir</a>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
        ...
        <div class="grid">
            ...
            <div class="card file-item" data-name="<?=esc($it)?>" data-isdir="<?= $isDir ? '1' : '0' ?>" data-open-url="<?= $isDir ? ('?path='.urlencode($next).'&view=grid') : esc(($rel? $rel.'/' : '').$it) ?>" data-delete-url="?path=<?=urlencode($rel)?>&view=grid&delete=<?=urlencode($it)?>">
                <div class="icon"><?= $isDir ? '📁' : '📄' ?></div>
                <div class="name"><?php if($isDir): ?><a href="?path=<?=urlencode($next)?>&view=grid"><?=esc($it)?></a><?php else: ?><a href="<?=esc(($rel? $rel.'/' : '').$it)?>" target="_blank"><?=esc($it)?></a><?php endif; ?></div>
                <div class="meta"><?= $isDir ? 'Pasta' : (filesize($full).' bytes') ?></div>
                <div class="action-row">
                    <a class="link-btn" href="#" onclick="return renameItem('<?=esc($it)?>')">Renomear</a>
                    <a class="danger" href="?path=<?=urlencode($rel)?>&view=grid&delete=<?=urlencode($it)?>" onclick="return confirm('Excluir este item?')">Excluir</a>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
