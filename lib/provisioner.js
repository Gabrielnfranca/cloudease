...
# Detectar PHP (Mover para o topo para garantir uso em blocos opcionais)
if ! command -v php &> /dev/null; then
    handle_error "PHP não encontrado no sistema"
fi
...
if ! command -v redis-server &> /dev/null; then 
    ...
    apt-get update -qq
    apt-get install -y -qq redis-server php\${PHP_VERSION}-redis || handle_error "Falha ao instalar Redis"
...
# Instalar Nginx se ausente
if ! command -v nginx &> /dev/null; then
    apt-get install -y nginx || handle_error "Falha ao instalar Nginx"
    ufw allow 'Nginx Full' || ufw allow 80
fi
...
` : ''}
` : `
# Site sem WP (PHP/HTML)
log "Configuração para PHP/HTML..."
if [ ! -f /var/www/${domain}/index.php ] && [ ! -f /var/www/${domain}/index.html ]; then
    echo "<?php echo '<h1>Site ${domain} instalado!</h1><p>Em breve seu conteúdo aqui.</p>'; phpinfo(); ?>" > /var/www/${domain}/index.php
fi
`}
