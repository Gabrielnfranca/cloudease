...
export async function provisionWordPress(serverIp, domain, wpConfig = null) {
    return new Promise((resolve, reject) => {
        ...
        // Mudança para /root para evitar problemas com noexec em /tmp e permissões
...
`;
        ...
        conn.on('ready', () => {
            ...
            // 1. Upload do Script (Via SFTP - Mais confiável que cat redirect)
            conn.sftp((err, sftp) => {
                if (err) {
                    conn.end();
                    return reject(new Error('[V2.2.5] Erro ao iniciar subsistema SFTP: ' + err.message));
                }
                ...
                stream.on('error', (err) => {
                    conn.end();
                    return reject(new Error('[V2.2.5] Erro durante escrita SFTP: ' + err.message));
                });

                stream.on('close', () => {
                    // Upload concluído, agora executa
...
export async function deleteSiteFromInstance(serverIp, domain, dbConfig = {}) {
    return new Promise((resolve, reject) => {
        ...
        const dbName = dbConfig.dbName || baseName;
        const dbUser = dbConfig.dbUser || dbName;
        const sysUser = dbConfig.sysUser || baseName;
        
        let privateKey;
        try { privateKey = getPrivateKey(); } catch (e) { return reject(e); }

        // Script de limpeza
        // userdel -r remove o usuário e seu home dir (/var/www/domain)
        // Adicionado || true para não falhar caso user já não exista
        const scriptContent = `
            # Mata processos do usuário primeiro para evitar erro "user currently used by process"
            pkill -u ${sysUser} || true
            
            # Remove usuário do sistema e diretório home
            userdel -r ${sysUser} || true
            groupdel ${sysUser} || true
            
            # Limpeza redundante de arquivos (caso userdel falhe ou pasta permaneça)
