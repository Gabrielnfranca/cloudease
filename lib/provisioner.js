...
$toggleView = $view === 'grid' ? 'list' : 'grid';
$siteName = '${domain}';
$base = realpath($ROOT);
...
<style>
    ...
    *{box-sizing:border-box}
    ...
    .icon-toggle{width:36px;height:36px;display:inline-flex;align-items:center;justify-content:center;border:1px solid #d1d5db;border-radius:8px;background:#fff;color:#374151;text-decoration:none;font-size:18px}
    .icon-toggle:hover{background:#f9fafb}
    ...
    <main class="main">
        <form class="toolbar" method="post" enctype="multipart/form-data">
            ...
            <div class="toolbar-right">
                ...
                <a class="icon-toggle" href="?path=<?=urlencode($rel)?>&view=<?=urlencode($toggleView)?>" title="<?= $view === 'grid' ? 'Mudar para detalhes' : 'Mudar para grade' ?>"><?= $view === 'grid' ? '☰' : '▦' ?></a>
            </div>
        </form>
