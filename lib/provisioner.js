...
`;
        ...
        conn.on('ready', () => {
            ...
            conn.exec(uploadCmd, (err, stream) => {
                if (err) { 
                    conn.end(); 
                    return reject(new Error('Erro ao solicitar upload: ' + err.message)); 
                }
                
                // IMPORTANTE: Aguardar término do upload antes de executar
        ...
        conn.on('ready', () => {
             conn.exec(cmd, (err, stream) => {
                 ...
                 stream.on('data', (data) => {
                     ...
                     if (code !== 0) {
                        // Certbot failure
                        return reject(new Error('Certbot falhou: ' + stderr));
                     }
                     resolve(true); 
                 });
             });
