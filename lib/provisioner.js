...
export async function provisionWordPress(serverIp, domain, wpConfig = null) {
    return new Promise((resolve, reject) => {
        ...
        let commands = [
            ...
            // Instalar Redis se solicitado (com modo não interativo para evitar travamentos)
            (wpConfig && wpConfig.cache === 'redis') ? 'export DEBIAN_FRONTEND=noninteractive && apt-get update -qq && apt-get install -y -qq redis-server php-redis && systemctl enable redis-server && systemctl start redis-server' : 'echo "Redis não solicitado"',
