...
<style>
    ...
    *{box-sizing:border-box}
    ...
    .breadcrumb-bar{background:#fff;border-bottom:1px solid var(--line);padding:8px 14px;display:flex;align-items:center;gap:8px;font-size:13px;min-height:32px}
    .breadcrumb-back{color:#6c4fd3;text-decoration:none;font-size:18px;font-weight:bold;padding:2px 6px;display:inline-flex;align-items:center;justify-content:center;hover:opacity:.7;transition:opacity .2s}
    .breadcrumb-path{display:flex;align-items:center;gap:4px;flex-wrap:wrap}
    .breadcrumb-item{color:#4338ca;text-decoration:none;padding:2px 6px;border-radius:5px;transition:background .2s}
    .breadcrumb-item:hover{background:#eef2ff}
    ...
    @media (max-width:900px){.shell{grid-template-columns:1fr}.sidebar{border-right:none;border-bottom:1px solid var(--line)}.toolbar{flex-direction:column;align-items:stretch}.toolbar-left,.toolbar-right{width:100%}.breadcrumb-bar{padding:8px 10px;font-size:12px}}
</style></head><body>
<div class="topbar">
...
<div class="breadcrumb-bar">
    ...
    <div class="breadcrumb-path">
        <a href="filemanager.php?view=<?=urlencode($view)?>&sort=<?=urlencode($sort)?>&order=<?=urlencode($order)?>" class="breadcrumb-item">Início</a>
        <?php 
        if($rel !== ''){
    ...
    <main class="main">
        ...
        <div class="list">
            <div class="list-header">
                <div>
                    <a class="sort-link <?= $sort === 'name' ? 'active' : '' ?>" href="?path=<?=urlencode($rel)?>&view=list&sort=name&order=<?=urlencode($sort === 'name' ? $nextOrder : 'asc')?>">Nome <span class="sort-arrow"><?= $sort === 'name' ? ($order === 'asc' ? '↑' : '↓') : '↕' ?></span></a>
                </div>
                <div>
                    <a class="sort-link <?= $sort === 'type' ? 'active' : '' ?>" href="?path=<?=urlencode($rel)?>&view=list&sort=type&order=<?=urlencode($sort === 'type' ? $nextOrder : 'asc')?>">Tipo <span class="sort-arrow"><?= $sort === 'type' ? ($order === 'asc' ? '↑' : '↓') : '↕' ?></span></a>
                </div>
                <div>
                    <a class="sort-link <?= $sort === 'size' ? 'active' : '' ?>" href="?path=<?=urlencode($rel)?>&view=list&sort=size&order=<?=urlencode($sort === 'size' ? $nextOrder : 'asc')?>">Tamanho <span class="sort-arrow"><?= $sort === 'size' ? ($order === 'asc' ? '↑' : '↓') : '↕' ?></span></a>
                </div>
                <div>
                    <a class="sort-link <?= $sort === 'date' ? 'active' : '' ?>" href="?path=<?=urlencode($rel)?>&view=list&sort=date&order=<?=urlencode($sort === 'date' ? $nextOrder : 'asc')?>">Data <span class="sort-arrow"><?= $sort === 'date' ? ($order === 'asc' ? '↑' : '↓') : '↕' ?></span></a>
                </div>
            </div>
