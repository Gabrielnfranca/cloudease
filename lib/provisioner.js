                 ...
                 stream.on('data', (data) => {
                     ...
                     if (code !== 0) {
                        ...
                        if (fullOutput.includes('DNS problem') || fullOutput.includes('NXDOMAIN')) {
                            ...
                            // Se não identificar padrão, pega as últimas linhas úteis (removendo log debug infos)
                            // Remove linhas "Saving debug log" e linhas vazias
                            const cleanLines = fullOutput.split('\n')
                                .filter(l => !l.includes('Saving debug log') && l.trim().length > 0);
                            errorMsg = cleanLines.slice(-2).join('. ') || fullOutput.slice(-200);
                        }
