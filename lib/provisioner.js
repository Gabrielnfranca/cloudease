...
<div class="topbar">
    <div><strong>CloudEase File Manager</strong> / <?=esc($rel ?: '/')?></div>
    <div class="actions">
...
<div class="shell">
    ...
    <main class="main">
        <form class="toolbar" method="post" enctype="multipart/form-data">
            ...
            <?php if($rel!==''){ $up = dirname($rel); if($up==='.')$up=''; echo '<a class="btn gray" href="?path='.urlencode($up).'">Voltar</a>'; } ?>
            <a class="btn gray" href="?path=<?=urlencode($rel)?>">Atualizar</a>
        </form>
        <?php $visibleItems = array_values(array_filter($items, fn($i) => $i !== '.' && $i !== '..')); ?>
        ...
        <div class="grid">
            ...
            <div class="card">
                ...
                <div class="name"><?php if($isDir): ?><a href="?path=<?=urlencode($next)?>"><?=esc($it)?></a><?php else: ?><a href="<?=esc(($rel? $rel.'/' : '').$it)?>" target="_blank"><?=esc($it)?></a><?php endif; ?></div>
                <div class="meta"><?= $isDir ? 'Pasta' : (filesize($full).' bytes') ?></div>
                <div><a class="danger" href="?path=<?=urlencode($rel)?>&delete=<?=urlencode($it)?>" onclick="return confirm('Excluir este item?')">Excluir</a></div>
            </div>
            <?php endforeach; ?>
        </div>
        <?php endif; ?>
