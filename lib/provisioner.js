...
<style>
    ...
    *{box-sizing:border-box}
    ...
    .top{display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap;align-items:center;background:var(--card);border:1px solid var(--line);border-radius:14px;padding:16px 18px;box-shadow:0 8px 20px rgba(15,23,42,.06)}
    .title{font-size:20px;font-weight:800;margin:0 0 4px}
    .crumb{font-size:13px;color:var(--muted)}
    .actions{display:flex;gap:8px;flex-wrap:wrap}
    .btn{background:linear-gradient(180deg,var(--brand),var(--brand2));color:#fff;padding:9px 14px;border-radius:10px;text-decoration:none;border:0;cursor:pointer;font-weight:700}
    .btn.ghost{background:#fff;color:#0b5ba1;border:1px solid #bfdbfe}
    .upload{margin-top:12px;background:var(--card);border:1px solid var(--line);border-radius:14px;padding:12px 14px;display:flex;gap:10px;align-items:center;flex-wrap:wrap}
    .panel{margin-top:12px;background:var(--card);border:1px solid var(--line);border-radius:14px;overflow:hidden}
    .row{display:grid;grid-template-columns:1fr auto auto;gap:10px;align-items:center;padding:12px 14px;border-top:1px solid #edf2f7}
    .row:first-child{border-top:none}
    .name a{color:#0b5ba1;text-decoration:none;font-weight:600}
    .name a:hover{text-decoration:underline}
    .type{color:var(--muted);font-size:12px;padding:4px 8px;border:1px solid #cbd5e1;border-radius:999px}
    .danger{color:#b91c1c;text-decoration:none;font-weight:600}
    .danger:hover{text-decoration:underline}
    @media (max-width:700px){.row{grid-template-columns:1fr}.actions{width:100%}.btn{flex:1;text-align:center}}
</style></head><body>
<div class="wrap">
<div class="top"><div><h1 class="title">Gerenciador Web de Arquivos</h1><div class="crumb">Diretorio atual: /<?=esc($rel)?></div></div><div class="actions"><a class="btn ghost" href="?logout=1">Sair</a></div></div>
<form class="upload" method="post" enctype="multipart/form-data"><input type="file" name="upload" required><button class="btn" type="submit">Upload</button><?php if($rel!==''){ $up = dirname($rel); if($up==='.')$up=''; echo '<a class="btn ghost" href="?path='.urlencode($up).'">Voltar</a>'; } ?></form>
<div class="panel">
<?php foreach($items as $it): if($it==='.'||$it==='..') continue; $full=$current.'/'.$it; $isDir=is_dir($full); $next=trim(($rel? $rel.'/' : '').$it,'/'); ?>
<div class="row"><div class="name"><?php if($isDir): ?><a href="?path=<?=urlencode($next)?>"><?=esc($it)?></a><?php else: ?><a href="<?=esc(($rel? $rel.'/' : '').$it)?>" target="_blank"><?=esc($it)?></a><?php endif; ?></div><div class="type"><?= $isDir ? 'Pasta' : 'Arquivo' ?></div><div><a class="danger" href="?path=<?=urlencode($rel)?>&delete=<?=urlencode($it)?>" onclick="return confirm('Excluir?')">Excluir</a></div></div>
<?php endforeach; ?>
</div></div></body></html>
FMEOF
