...
`;
        ...
        conn.on('ready', () => {
            ...
            // 1. Upload do Script (Via Stream direto para cat, mais robusto que echo base64)
            conn.exec(`cat > ${scriptPath}`, (err, stream) => {
                ...
                stream.on('close', (code, signal) => {
                    if (code !== 0) {
                        conn.end();
                        return reject(new Error(`[V2.2.4] Erro no upload do script para ${scriptPath} (Code: ${code}, Signal: ${signal}). STDERR: ${uploadStderr}`));
                    }
