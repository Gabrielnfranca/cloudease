...
<div class="shell">
    <aside class="sidebar">
        ...
        <ul class="tree">
            <li><a href="filemanager.php?view=<?=urlencode($view)?>&sort=<?=urlencode($sort)?>&order=<?=urlencode($order)?>">📁 /</a></li>
            <?php foreach($treeItems as $dir): if($dir==='.'||$dir==='..') continue; $fullBase=$base.'/'.$dir; if(!is_dir($fullBase)) continue; ?>
            <li><a href="?path=<?=urlencode($dir)?>&view=<?=urlencode($view)?>&sort=<?=urlencode($sort)?>&order=<?=urlencode($order)?>">📁 <?=esc($dir)?></a></li>
            <?php endforeach; ?>
        </ul>
    </aside>
    <main class="main">
        <form class="toolbar" method="post" enctype="multipart/form-data">
            ...
            <div class="toolbar-right">
                <?php if($rel!==''){ $up = dirname($rel); if($up==='.')$up=''; echo '<a class="btn gray" href="?path='.urlencode($up).'&view='.urlencode($view).'&sort='.urlencode($sort).'&order='.urlencode($order).'">Voltar</a>'; } ?>
                <a class="btn gray" href="?path=<?=urlencode($rel)?>&view=<?=urlencode($view)?>&sort=<?=urlencode($sort)?>&order=<?=urlencode($order)?>">Atualizar</a>
                <a class="btn gray" href="?path=<?=urlencode($rel)?>&view=<?=urlencode($view)?>&sort=name&order=<?=urlencode($order)?>">Nome</a>
                <a class="btn gray" href="?path=<?=urlencode($rel)?>&view=<?=urlencode($view)?>&sort=type&order=<?=urlencode($order)?>">Tipo</a>
                <a class="btn gray" href="?path=<?=urlencode($rel)?>&view=<?=urlencode($view)?>&sort=size&order=<?=urlencode($order)?>">Tamanho</a>
                <a class="btn gray" href="?path=<?=urlencode($rel)?>&view=<?=urlencode($view)?>&sort=date&order=<?=urlencode($order)?>">Data</a>
                <a class="btn gray" href="?path=<?=urlencode($rel)?>&view=<?=urlencode($view)?>&sort=<?=urlencode($sort)?>&order=<?=urlencode($nextOrder)?>" title="Alternar ordem"><?= $order === 'asc' ? '↑' : '↓' ?></a>
        ...
        <div class="list">
            ...
            <div class="list-row file-item" data-name="<?=esc($it)?>" data-isdir="<?= $isDir ? '1' : '0' ?>" data-open-url="<?= $isDir ? ('?path='.urlencode($next).'&view=list&sort='.urlencode($sort).'&order='.urlencode($order)) : esc(($rel? $rel.'/' : '').$it) ?>" data-delete-url="?path=<?=urlencode($rel)?>&view=list&sort=<?=urlencode($sort)?>&order=<?=urlencode($order)?>&delete=<?=urlencode($it)?>">
                ...
                <div>
                    ...
                    <a class="danger" href="?path=<?=urlencode($rel)?>&view=list&sort=<?=urlencode($sort)?>&order=<?=urlencode($order)?>&delete=<?=urlencode($it)?>" onclick="return confirm('Excluir este item?')">Excluir</a>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
        ...
        <div class="grid">
            ...
            <div class="card file-item" data-name="<?=esc($it)?>" data-isdir="<?= $isDir ? '1' : '0' ?>" data-open-url="<?= $isDir ? ('?path='.urlencode($next).'&view=grid&sort='.urlencode($sort).'&order='.urlencode($order)) : esc(($rel? $rel.'/' : '').$it) ?>" data-delete-url="?path=<?=urlencode($rel)?>&view=grid&sort=<?=urlencode($sort)?>&order=<?=urlencode($order)?>&delete=<?=urlencode($it)?>">
