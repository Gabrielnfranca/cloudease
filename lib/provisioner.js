...
export async function updateNginxConfig(serverIp, domain, enableTempUrl, phpVersion) {
    return new Promise((resolve, reject) => {
        ...
        // Tenta extrair a versão numérica (ex: "PHP 8.2" -> "8.2")
        // Default para 8.2 se não identificar
        let ver = "8.2"; 
        if (phpVersion) {
...
server {
    ...
    location ~ \\.php$ { include snippets/fastcgi-php.conf; fastcgi_pass unix:/var/run/php/php${ver}-fpm.sock; }
}
EOF
        nginx -t && systemctl reload nginx
        `;
