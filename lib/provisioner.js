...
if ! mysql -e "SELECT 1" &> /dev/null; then
    ...
    # Reseta a senha do root para usar plugin unix_socket
    # Tenta sintaxe MySQL 5.7+ / MariaDB 10+
    mysql -e "FLUSH PRIVILEGES; UPDATE mysql.user SET plugin='unix_socket' WHERE User='root'; FLUSH PRIVILEGES;" || \
    mysql -e "FLUSH PRIVILEGES; UPDATE mysql.user SET plugin='auth_socket' WHERE User='root'; FLUSH PRIVILEGES;" || \
    mysql -e "FLUSH PRIVILEGES; ALTER USER 'root'@'localhost' IDENTIFIED VIA unix_socket; FLUSH PRIVILEGES;" || true
