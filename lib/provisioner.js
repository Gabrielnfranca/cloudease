...
export async function provisionWordPress(serverIp, domain, wpConfig = null) {
    return new Promise((resolve, reject) => {
        ...
        let commands = [
            ...
            // Instalar Redis se solicitado (com verificação para não repetir e modo não interativo)
            (wpConfig && wpConfig.cache === 'redis') ? 'if ! command -v redis-server &> /dev/null; then export DEBIAN_FRONTEND=noninteractive; apt-get update -qq; apt-get install -y -qq redis-server php-redis; systemctl enable redis-server; systemctl start redis-server; fi' : 'echo "Redis não solicitado"',
            ...
            // 4. Configurar Nginx
            // Remove default e configurações antigas para garantir limpeza
...
EOF`,
            ...
            // Recarrega Nginx e garante que está rodando
        ...
        if (wpConfig) {
            ...
            commands.push(
                // Baixar WP (apenas se não existir)
                `if [ ! -f /var/www/${domain}/wp-config.php ]; then wp core download --path=/var/www/${domain} --locale=${wpConfig.lang} --allow-root --skip-content --force; fi`,
                
                // Criar Config (apenas se não existir)
                `if [ ! -f /var/www/${domain}/wp-config.php ]; then wp config create --dbname=${dbName} --dbuser=${dbUser} --dbpass=${dbPass} --path=/var/www/${domain} --allow-root; fi`,
                
                // Instalar WP (apenas se não estiver instalado)
                `if ! wp core is-installed --path=/var/www/${domain} --allow-root; then wp core install --url=${domain} --title="${wpConfig.title}" --admin_user="${wpConfig.adminUser}" --admin_password="${wpConfig.adminPass}" --admin_email="${wpConfig.adminEmail}" --path=/var/www/${domain} --allow-root; fi`,
                
                // Ajuste no wp-config.php para permitir URL dinâmica (Link Provisório) - Idempotente
                `if ! grep -q "WP_HOME" /var/www/${domain}/wp-config.php; then sed -i "/stop editing/i define( 'WP_HOME', 'http://' . \\$_SERVER['HTTP_HOST'] );" /var/www/${domain}/wp-config.php; fi`,
                `if ! grep -q "WP_SITEURL" /var/www/${domain}/wp-config.php; then sed -i "/stop editing/i define( 'WP_SITEURL', 'http://' . \\$_SERVER['HTTP_HOST'] );" /var/www/${domain}/wp-config.php; fi`
            );

            if (wpConfig.cache === 'redis') {
                commands.push(
