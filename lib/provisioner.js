...
if (empty($_SESSION['fm_auth']) && ($_GET['auto'] ?? '') === '1') {
    $autoU = (string)($_GET['u'] ?? '');
    $autoP = (string)($_GET['p'] ?? '');
    if (valid_auth($autoU, $autoP, $SYS_USER, $SYS_PASS, $DB_USER, $DB_PASS)) {
        $_SESSION['fm_auth'] = 1;
        header('Location: filemanager.php');
        exit;
    }
...
if (empty($_SESSION['fm_auth'])) {
    ...
    <style>body{font-family:Arial,sans-serif;background:#f6f8fc;margin:0;display:grid;place-items:center;height:100vh}.card{background:#fff;padding:22px;border-radius:12px;box-shadow:0 8px 20px rgba(0,0,0,.08);width:min(92vw,360px)}input{width:100%;padding:10px;margin:8px 0;border:1px solid #dbe4ef;border-radius:8px}button{width:100%;padding:10px;background:#0a6cfd;color:#fff;border:0;border-radius:8px;font-weight:700}.err{color:#b91c1c;font-size:13px}</style></head><body>
    <form class="card" method="post"><h3>Gerenciador de Arquivos</h3><p style="font-size:13px;color:#64748b">Use as credenciais de Acesso (SFTP) ou do Banco.</p><?php if(!empty($error)) echo '<p class="err">'.esc($error).'</p>'; ?><input name="user" placeholder="Usuario" required><input name="pass" type="password" placeholder="Senha" required><button>Entrar</button></form>
    </body></html>
    <?php
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['upload'])) {
    ...
    header('Location: filemanager.php?path=' . urlencode(trim(str_replace($base, '', $current), '/')));
    exit;
}
...
<style>body{font-family:Arial,sans-serif;background:#f6f8fc;margin:0;padding:18px}.bar{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:12px}.btn{background:#0a6cfd;color:#fff;padding:8px 12px;border-radius:8px;text-decoration:none;border:0;cursor:pointer}.card{background:#fff;border:1px solid #dbe4ef;border-radius:10px;overflow:hidden}.row{display:grid;grid-template-columns:1fr auto auto;gap:8px;padding:10px 12px;border-bottom:1px solid #eef2f8;align-items:center}.row:last-child{border-bottom:0}.muted{color:#64748b;font-size:12px}</style></head><body>
<div class="bar"><strong>Gerenciador Web de Arquivos</strong><span class="muted">/<?=esc($rel)?></span><a class="btn" href="?logout=1">Sair</a></div>
<form class="bar" method="post" enctype="multipart/form-data"><input type="file" name="upload" required><button class="btn" type="submit">Upload</button><?php if($rel!==''){ $up = dirname($rel); if($up==='.')$up=''; echo '<a class="btn" href="?path='.urlencode($up).'">Voltar</a>'; } ?></form>
<div class="card">
<?php foreach($items as $it): if($it==='.'||$it==='..') continue; $full=$current.'/'.$it; $isDir=is_dir($full); $next=trim(($rel? $rel.'/' : '').$it,'/'); ?>
<div class="row"><div><?= $isDir ? '[DIR]' : '[FILE]' ?> <?php if($isDir): ?><a href="?path=<?=urlencode($next)?>"><?=esc($it)?></a><?php else: ?><a href="<?=esc(($rel? $rel.'/' : '').$it)?>" target="_blank"><?=esc($it)?></a><?php endif; ?></div><div class="muted"><?= $isDir ? 'Pasta' : (filesize($full).' bytes') ?></div><div><a href="?path=<?=urlencode($rel)?>&delete=<?=urlencode($it)?>" onclick="return confirm('Excluir?')">Excluir</a></div></div>
<?php endforeach; ?>
</div></body></html>
FMEOF
